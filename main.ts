...
server.tool(
  ...
  async ({ city }) => {
    try {
        ...
        const weatherResponse = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,weathercode,precipitation,is_day,rain&timezone=auto`);
